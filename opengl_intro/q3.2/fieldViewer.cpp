...
float cmplx_sq_x (float z_x, float z_y)
{
  return (z_x*z_x - z_y*z_y);
}

float cmplx_sq_y (float z_x, float z_y)
{
  return (2*z_x*z_y);
}

float f_real(float z_x, float z_y)
{
  float z2_x = cmplx_sq_x (z_x, z_y);
  float z2_y = cmplx_sq_y (z_x, z_y);

  float z4_x = cmplx_sq_x (z2_x, z2_y);
  float z4_y = cmplx_sq_y (z2_x, z2_y);

  return (z_x*z4_x - z_y*z4_y - pow(1.01,4)*z_x);

}

float f_imag(float z_x, float z_y)
{
  float z2_x = cmplx_sq_x (z_x, z_y);
  float z2_y = cmplx_sq_y (z_x, z_y);
  ...
  return (z_y*z4_x + z4_y*z_x - pow(1.01,4)*z_y);

}

float f_real_8(float z_x, float z_y)
{
  float z2_x = cmplx_sq_x (z_x, z_y);
  float z2_y = cmplx_sq_y (z_x, z_y);
  ...
  float z8_x = cmplx_sq_x (z4_x, z4_y);
  float z8_y = cmplx_sq_y (z4_x, z4_y);

  return (z_x*z8_x - z_y*z8_y - pow(1.01,8)*z_x);

}

float f_imag_8(float z_x, float z_y)
{
  float z2_x = cmplx_sq_x (z_x, z_y);
  float z2_y = cmplx_sq_y (z_x, z_y);
  ...
  return (z_y*z8_x + z8_y*z_x - pow(1.01,8)*z_y);
...
void runOnce()
{
  ...
  for (int y = 0; y < field.yRes(); y++)
    for (int x = 0; x < field.xRes(); x++)
    {
      ...
      float c_x = scale * (xReal + shift); 
      float c_y = scale * (yReal + shift);

      // float modq = sqrt(q_x*q_x + q_y*q_y);
      float q_x = 0;
      float q_y = 0;
      float q_x_temp = 0;
      float modq = 0;
      ...
      i = 0;
      while (i<maxiter && modq<=maxradius)
      {

        q_x_temp = f_real(q_x, q_y) + c_x;
        q_y = f_imag(q_x,q_y) + c_y;
        q_x = q_x_temp;

        modq = sqrt(q_x*q_x + q_y*q_y);
        i++;
      }

      if (i == maxiter ) 
      {
        field(x,y) = VEC3F(1.0, 0.0, 0.0);
      }
      else
      {
        field(x,y) = VEC3F(1.0-float(colorscale/i),1.0-float(colorscale/i),1.0-float(colorscale/i));
      }
